...
#[derive(Deserialize)]
struct Args { path: String, max_depth: Option<usize> }
...
impl Tool for ListDirectory {
    ...
    async fn execute(&self, args: Value, ctx: &ToolContext) -> anyhow::Result<ToolOutput> {
        ...
        for e in walkdir::WalkDir::new(&base).max_depth(depth).into_iter().filter_map(|e| e.ok()) {
            let rel = e.path().strip_prefix(&base).unwrap_or(e.path()).display().to_string();
            let kind = if e.file_type().is_dir() { "dir" } else { "file" };
            let line = format!("{} {}\n", kind, rel);
            total += line.len();
            if total > ctx.max_output_bytes { truncated = true; break; }
            out.push_str(&line);
...
mod tests {
    ...
    async fn lists_temp_dir() {
        ...
        let t = ListDirectory;
        let out = t.execute(json!({"path":"."}), &ctx(tmp.path().to_path_buf())).await.unwrap();
