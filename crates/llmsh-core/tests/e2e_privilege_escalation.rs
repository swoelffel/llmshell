//! E2E: a direct `sudo …` invocation reaches `ConfirmStrong` with phrase,
//! audit log records `UsesPrivilegeEscalation` and `effective_risk:"privileged"`.
...
#[tokio::test]
async fn direct_sudo_reaches_confirm_strong() {
    let scripted = vec![LlmResponse {
        ...
        tool_calls: vec![ToolCall {
            ...
            args: json!({
                "program":"sudo",
                "args":["softwareupdate","--install","--all"],
    ...
    let res = agent.run("install updates").await.unwrap();
    ...
    assert!(
        ...
        "expected UsesPrivilegeEscalation reason:\n{log}"
    );
    assert!(
