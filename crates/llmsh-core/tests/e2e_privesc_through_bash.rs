//! E2E regression: `bash -c "sudo …"` ALSO reaches `ConfirmStrong` after the
//! deshell pass detects privilege escalation in the wrapped command.
...
#[tokio::test]
async fn bash_wrapped_sudo_reaches_confirm_strong() {
    let scripted = vec![LlmResponse {
        ...
        tool_calls: vec![ToolCall {
            ...
            args: json!({
                "program":"bash",
                "args":["-c", "sudo softwareupdate --install --all"],
                "intent":"install all macOS updates",
                "claimed_risk":"write"
            }),
        }],
    ...
    let res = agent.run("install updates via bash").await.unwrap();
    ...
    assert!(
        log.contains("\"UsesPrivilegeEscalation\""),
        "expected UsesPrivilegeEscalation reason after deshell:\n{log}"
    );
    assert!(
        log.contains("\"effective_risk\":\"privileged\""),
        "expected privileged classification:\n{log}"
    );
    assert!(
        log.contains("\"strong\":true"),
        "expected ConfirmStrong action recorded:\n{log}"
    );
}
