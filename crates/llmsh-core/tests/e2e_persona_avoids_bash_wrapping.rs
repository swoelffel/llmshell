//! E2E persona regression guard: argv-direct `dscl` (the persona-prescribed
//! shape) classifies `ReadOnly` and never prompts. The actual command may not
//! exist on Linux CI — we ignore the run result and assert only on the audit
//! log, since the policy decision is recorded BEFORE execution.
...
#[tokio::test]
async fn argv_direct_dscl_classifies_readonly() {
    let scripted = vec![
        LlmResponse {
            message: None,
    ...
    // Ignore run result: on Linux CI `dscl` may not exist, but the policy
    // decision is recorded BEFORE execution, so the audit assertions still hold.
    let _ = agent.run("list local users").await;
    ...
    assert!(
        ...
        "expected readonly classification:\n{log}"
    );
    assert!(
