...
impl PolicyEngine for DefaultPolicyEngine {
    fn evaluate(&self, call: &CheckedToolCall, _ctx: &PolicyContext) -> PolicyDecision {
        ...
        if has_sensitive && self.config.sensitive_paths_action == RiskAction::Deny {
        ...
        let has_outside = call.resolved_paths.iter().any(|p| !p.inside_workspace);
        if has_outside {
            flags.push(PolicyFlag::OutsideWorkspace);
            reasons.push("path outside workspace".into());
